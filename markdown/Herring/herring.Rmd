...
```
  <em>Note: The currently used settings has moch lower AIC, meaning that the model fits data significantly better.</em>
</details>
...
```r
  ...
  resCurrent = residuals(fitCurrent)  #TODO: Something wrong when fixing sd_N.
  plot(resStandard)
  plot(resCurrent)
```
